resource "datadog_monitor" "dynamo_returned_item_count" {
  ...
  message = <<EOF
  Returned item count for last ${var.period} on host {host.name} ({host.ip})
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Returned item count for last ${var.period} on host {host.name} ({host.ip}) ${var.escalation_notify}"

resource "datadog_monitor" "dynamo_returned_item_count" {
  ...
  message = <<EOF
  Returned item count for last ${var.period} on table {bucketname.name}
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Returned item count for last ${var.period} on table {bucketname.name} ${var.escalation_notify}"

resource "datadog_monitor" "dynamo_throttled_requests" {
  ...
  message = <<EOF
  Throttled requests for last ${var.period} on host {host.name} ({host.ip})
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Throttled requests for last ${var.period} on host {host.name} ({host.ip}) ${var.escalation_notify}"

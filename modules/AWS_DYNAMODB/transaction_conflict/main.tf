resource "datadog_monitor" "dynamo_transaction_conflict" {
  ...
  message = <<EOF
  Transaction conflict for last ${var.period} on table {bucketname.name}
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Transaction conflict for last ${var.period} on table {bucketname.name} ${var.escalation_notify}"

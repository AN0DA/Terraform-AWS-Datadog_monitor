resource "datadog_monitor" "dynamo_account_provisioned_read_capacity_utilization" {
  ...
  message = <<EOF
  Account provisioned read capacity utilization for last ${var.period} on fuction {bucketname.name}
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Account provisioned read capacity utilization for last ${var.period} on fuction {bucketname.name} ${var.escalation_notify}"

resource "datadog_monitor" "rds_write_iops" {
  ...
  message = <<EOF
  Write IO operations for last ${var.period} on bucket {bucketname.name}
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Write IO operations for last ${var.period} on bucket {bucketname.name} ${var.escalation_notify}"

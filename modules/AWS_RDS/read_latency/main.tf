resource "datadog_monitor" "rds_read_latency" {
  ...
  message = <<EOF
  Read latency for last ${var.period} on bucket {bucketname.name}
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Read latency for last ${var.period} on bucket {bucketname.name} ${var.escalation_notify}"

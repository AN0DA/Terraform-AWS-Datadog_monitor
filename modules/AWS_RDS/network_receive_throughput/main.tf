resource "datadog_monitor" "rds_network_receive_throughput" {
  ...
  message = <<EOF
  Network receive throughput for last ${var.period} on host {host.name} ({host.ip})
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Network receive throughput for last ${var.period} on host {host.name} ({host.ip}) ${var.escalation_notify}"

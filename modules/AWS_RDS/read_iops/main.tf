resource "datadog_monitor" "rds_read_iops" {
  ...
  message = <<EOF
  Read IO operations for last ${var.period} on host {host.name} ({host.ip})
  ${var.remediation}
  ${var.notify}
  EOF

  escalation_message = "Read IO operations for last ${var.period} on host {host.name} ({host.ip}) ${var.escalation_notify}"
